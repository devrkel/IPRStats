...
import wx
import string, os, wx.grid, ConfigParser
...
from importers.ebixml import EBIXML
...
class IPRStats_Frame(wx.Frame):
    def __init__(self, *args, **kwds):
        ...
        if not os.path.exists(self.sessiondir):
            try: os.mkdir(self.sessiondir)
            except: self.sessiondir = tempfile.gettempdir()
        
        # Open and parse settings file
        self.configpath = os.path.join(homedir,'.iprstats','iprstats.cfg')
        ...
        self.open_item = wx.MenuItem(self.file_item, wx.ID_OPEN,
                                     "&Open...", "", wx.ITEM_NORMAL)
        self.export_html_item = wx.MenuItem(self.file_item, wx.NewId(),
                                            "Export as &HTML...", "", wx.ITEM_NORMAL)
        self.export_xls_item = wx.MenuItem(self.file_item, wx.NewId(), "Export as &XLS...", "", wx.ITEM_NORMAL)
        self.properties_item = wx.MenuItem(self.file_item, wx.ID_PROPERTIES, "&Properties....", "", wx.ITEM_NORMAL)
        ...
        self.file_item.AppendItem(self.properties_item)
...
""")
        ...
        self.Bind(wx.EVT_MENU, self.OnSettings, self.properties_item)
    ...
    def OnSettings(self, event):
        ...
        if dlg.ShowModal() == wx.ID_OK:
            ...
            self.config.set('general', 'chart_type', dlg.ChartTypeCmb.GetValue().lower())
    ...
    def PopulateGUI(self):
        ...
        for app in self.apps:
            ...
            if ipsstat.get_chart(counts, app+' Matches', chart_filename, self.config.get('general', 'chart_type')):
